...
export function analyzeDraft(description) {
  ...
  const missingFields = fieldQuestions.filter(([field]) => !known[field]).map(([field]) => field);
  const questions = fieldQuestions.filter(([field]) => missingFields.includes(field)).slice(0, 4).map(([field, text]) => ({ field, text }));
  while (questions.length < 3) { const [field, text] = fieldQuestions[questions.length]; questions.push({ field, text }); }
